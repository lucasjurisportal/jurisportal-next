...
import { createGlobalWorkItem } from "@/modules/work-items/application/work-item-service";
import { globalWorkItemCreateSchema } from "@/modules/work-items/domain/work-item.schema";
...
export async function POST(request: Request) {
  ...
  const parsed = globalWorkItemCreateSchema.safeParse(body);
  if (!parsed.success) return NextResponse.json({ error: "INVALID_WORK_ITEM", fields: parsed.error.flatten().fieldErrors }, { status: 422 });
  try {
    const item = await createGlobalWorkItem({ organizationId: context.workspace.organizationId, actorUserId: context.user.id, data: parsed.data });
    try { await syncWorkItemToGoogleCalendar({ organizationId: context.workspace.organizationId, workItemId: item.id }); } catch (error) { console.warn("[google-calendar.work-item.create]", error); }
    ...
    console.error("[work-items.create]", error);
