...
function processMutablePersistence(input: ProcessInput) {
  return {
    ...
    caseType: optional(input.caseType),
    processClass: optional(input.processClass),
    subject: optional(input.subject),
    otherSubjects: [...new Set(input.otherSubjects.map((subject) => subject.trim()).filter(Boolean))]
      .filter((subject) => subject !== input.subject?.trim()),
